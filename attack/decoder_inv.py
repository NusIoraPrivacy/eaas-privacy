...
def train_denoise(base_model, tokenizer=None, args=None):
    ...
    if args.denoise_data == "synthetic":
        ...
        train_dataset = UniversalDataset(args.denoise_data, 'train', tokenizer, SAMPLE_SIZE, args)
        print("train_dataset_length: ", len(train_dataset))
    ...
    denoise_mod = serverDenoiseModel(d_model=embed_dim, args=args)
    ...
    for epoch in epoch_pbar:
        with tqdm(total=len(dataloader), desc=f"Epoch {epoch+1} Batches") as pbar:
            for i, batch in enumerate(tqdm(dataloader, desc=f"Epoch {epoch+1} Batches"), start=1):
                ...
                for j in range(args.noise_per_sample):
                    ...
                    if eta > 0:
                        ...
                        noise_init_emb = sample_noise_Chi(init_emb.shape, 50, args.device)
                    
                    # training denoise model
                    optimizer.zero_grad()
                    output = denoise_mod(noise_init_emb, attention_masks, input_ids)
                    loss = output.loss
                    loss.backward() 
                    optimizer.step()
...
def test_denoise(args, denoise_model=None):
    ...
    embed_dim = emb_size_dict[args.base_model]
    # Load the trained denoise model
    d_epoch = args.denoise_epochs-1 if denoise_model == None else 1

    print(f"Test for denoise model of epoch {d_epoch}")
    # load denoise model
    if denoise_model == None:
        denoise_model = serverDenoiseModel(d_model=embed_dim, args=args)  # Set the required parameters
        base_mod_name = args.base_model.split('/')[-1]
        if args.denoise_data == "mix":
            denoise_data_name = args.mixed_data_config_list.split(",")
            denoise_data_name = [i.strip()[0] for i in denoise_data_name]
    ...
    with tqdm(
        ...
        for i, batch in enumerate(tqdm(dataloader, desc=f"Epoch {d_epoch+1} Batches"), start=1):
            ...
            # test denoise model
            outputs = denoise_model(noise_init_emb, attention_masks) # [batch size, vob size, token length]
...
if __name__ == "__main__":
    ...
    for model in models:
        ...
        for eta in [100, 500]:
            ...
            print(f"Testing for model {model} under eta {args.train_eta}")
            # tokenizer, base_model = get_pretrained_model(args)
            # denoise_mod = train_denoise(base_model= base_model, args=args, tokenizer=tokenizer)
            args.base_batch_size = 10
            # test_denoise(args, denoise_mod)
            test_denoise(args)
            print(f"Finish testing for model {model} eta {args.train_eta}")