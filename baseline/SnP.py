...
from util.utils import text2text_priv, text2text_priv_ui, get_pretrained_model, get_cls_embedding, get_ui_top
import os
...
raw_train_dataset = DownStreamDataset('train', tokenizer, args)
train_dataloader = DataLoader(raw_train_dataset, batch_size=1000, shuffle=False, num_workers=0, drop_last=True)
top_ui_dict = get_ui_top(train_dataloader, tokenizer, args)
# print(top_ui_dict)
...
for eta in dp_levels:
    ...
    # for inputs in raw_train_inputs:
    #     if eta != 0:
    #         inputs = text2text_priv_ui(inputs, train_labels, tokenizer, base_model, top_ui_dict, args)
    ...
    train_cls_embs = torch.load(os.path.join(save_dir, f'train_cls_embs.pt'))
    test_cls_embs = torch.load(os.path.join(save_dir, f'test_cls_embs.pt'))
